...
pub(crate) fn wait_done_worker(name: &str, opts: Vec<worker::Opt>) -> Node {
    worker::Spec::new(name, opts, |ctx: Context| async move {
...
pub(crate) fn fail_start_worker(name: &str, opts: Vec<worker::Opt>) -> Node {
...
pub(crate) fn never_start_worker(name: &str, opts: Vec<worker::Opt>) -> Node {
    worker::Spec::new_with_start(
        name,
        opts,
...
/// its supervisor.
pub(crate) fn never_terminate_worker(name: &str, opts: Vec<worker::Opt>) -> Node {
    worker::Spec::new(name, opts, |_ctx: Context| async move {
