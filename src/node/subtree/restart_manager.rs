...
impl RestartManager {
    ...
    /// surpassed.
    pub(crate) fn register_error(mut self, err: anyhow::Error) -> Result<Self, ToManyRestarts> {
...
#[cfg(test)]
mod tests {
    use super::{RestartManager, ToManyRestarts};
    use tokio::time;

    #[tokio::test]
    async fn test_restart_manager_to_many_restarts() {
        time::pause();
        ...
        let err: ToManyRestarts = manager
